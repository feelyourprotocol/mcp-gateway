...
describe("toMcpError", () => {
  ...
  it("maps ZodError to invalid_input", () => {
    ...
    expect(result.isError).toBe(true);
    expect(extractTextContent(result)).toBe(
      "bytecode required (invalid_input)",
    );
  });

  it("maps EngineError to isError with engine code", () => {
    const result = toMcpToolError(
      new EngineError("Bytecode too large", "bytecode_too_large"),
    );

    expect(result.isError).toBe(true);
    expect(extractTextContent(result)).toBe(
      "Bytecode too large (bytecode_too_large)",
    );
  });
  ...
  it("runToolHandler catches EngineError into MCP isError", async () => {
    const result = await runToolHandler(async () => {
      throw new EngineError(
        "Unsupported base hardfork: bogus",
