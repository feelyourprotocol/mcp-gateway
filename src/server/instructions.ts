...
export const SERVER_INSTRUCTIONS = [
  ...
  'Each eips[] row includes specUrl, specDate, status, and testReleaseName (EST tag) or a live EIP page when unpinned.',
  'When the user asks about a specific EIP, cite that snapshot once in plain language (status, date, named test release);',
  'do not repeat it on every later result unless they ask what the answer is based on.',
  'Named eips:[NNNN] runs also put a compact Spec: clause on provenance.caveat; generic fork runs keep perEip as the machine record only.',
